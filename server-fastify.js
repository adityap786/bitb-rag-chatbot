...
class VectorStoreAdapter {
  ...
  async addDocuments(trialToken, documents) {
    ...
    indexes.set(trialToken, {
      vectors: documents.map(() => Array(384).fill(0)), // Mock 384-dim vectors
      metadata: documents,
 ...
 */
fastify.post('/api/start-trial', { schema: startTrialSchema }, async (request, reply) => {
  ...
  // For production JWT:
  // const trial_token = fastify.jwt.sign({
  //   aud: site_origin,
  //   sub: admin_email,
  //   trial: true
  // }, { expiresIn: '3d' });
...
fastify.post('/api/ingest', async (request, reply) => {
  ...
  // Extract trial_token from multipart fields or body
  ...
  if (data) {
    ...
    for await (const part of request.parts()) {
      if (part.type === 'field' && part.fieldname === 'trial_token') {
        trial_token = part.value;
      } else if (part.type === 'field' && part.fieldname === 'site_url') {
        site_url = part.value;
 ...
 */
fastify.get('/api/ingest/status/:job_id', async (request, reply) => {
  ...
  if (job.status === 'completed') {
    const index = indexes.get(job.trial_token);
    response.index_path = index?.index_path || null;
    response.documents_count = index?.metadata.length || 0;
  }
 ...
 */
fastify.get('/api/check-trial', async (request, reply) => {
 ...
 */
fastify.get('/api/voicesupport', async (request, reply) => {
  return {
